...
(function () {
    ...
    const BaqdDB = {
        ...
        /** Save/update a user using their Auth UID or encoded email for consistency */
        async saveUser(user) {
            if (!user || !user.email) return;

            // Get UID from current Auth session or fallback to encoded email
            const currentUser = firebase.auth().currentUser;
            const uid = currentUser ? currentUser.uid : (user.uid || 'temp_' + _enc(user.email));

            // LS update
            const users = _ls('baqdouns_users');
            const idx = users.findIndex(u => u.email === user.email);
            const userWithUid = { ...user, uid };
            if (idx > -1) users[idx] = { ...users[idx], ...userWithUid };
            else users.push(userWithUid);
            _lsSet('baqdouns_users', users);

            // Firebase update
            if (!_db) return;
            try {
                // 1. Map email to UID for lookups
                await _db.ref(`email_to_uid/${_enc(user.email)}`).set(uid);

                // 2. Save complete user object under the UID node
                await _db.ref(`users/${uid}`).update(userWithUid);

                // 3. (Legacy/Backup) Also save under encoded email for easier admin access if needed
                // but we prefer UID. However, to ensure admin sees it in all versions:
                await _db.ref(`users/${_enc(user.email)}`).update(userWithUid);

                console.log(`✅ BaqdDB: User ${user.email} synced to Firebase (UID: ${uid})`);
            } catch (e) {
                console.warn('BaqdDB.saveUser Firebase error:', e.message);
            }
        },
        ...
        async updateUser(email, updates) {
            ...
            if (!_db) return;
            try {
                // Try to find the real UID first
                if (!uid) {
                    const uidSnap = await _db.ref(`email_to_uid/${_enc(email)}`).get();
                    uid = uidSnap.exists() ? uidSnap.val() : 'temp_' + _enc(email);
                }

                // Update in both potential locations to be absolutely sure
                const updates_with_uid = { ...updates, updated: Date.now() };
                await _db.ref(`users/${uid}`).update(updates_with_uid);
                await _db.ref(`users/${_enc(email)}`).update(updates_with_uid);
        ...
        async getUsers() {
            ...
            try {
                ...
                // De-duplicate users (since they might be saved under both UID and email keys)
                const userMap = {};
                Object.values(data).forEach(u => {
                    if (u && u.email && u.email !== 'undefined') {
                        const emailLower = u.email.toLowerCase();
                        // If we have duplicates, keep the one that has a googleId or more points
                        if (!userMap[emailLower] || u.googleId || (u.points > (userMap[emailLower].points || 0))) {
                            userMap[emailLower] = u;
                        }
                    }
                });

                let fbUsers = Object.values(userMap);
        ...
        listenUsers(callback) {
            ...
            const handler = async (snap) => {
                ...
                Object.values(data).forEach(u => {
                    if (u && u.email && u.email !== 'undefined') {
                        const emailLower = u.email.toLowerCase();
                        if (!userMap[emailLower] || u.googleId) {
                            userMap[emailLower] = u;
                        }
                    }
                });
        ...
        // ════════════════════════════════════════
        // 🔄 DEEP SYNC & RECOVERY (Bring missing accounts)
        // ════════════════════════════════════════

        /** Deeply sync users from all possible Firebase paths (Recovery Tool) */
        async deepSyncUsers() {
            if (!_db) return { success: false, count: 0 };
            console.log('🔍 BaqdDB: Scanning for missing accounts...');
            try {
                const [usersSnap, emailToUidSnap, loginLogsSnap] = await Promise.all([
                    _db.ref('users').get(),
                    _db.ref('email_to_uid').get(),
                    _db.ref('login_logs').limitToLast(2000).get()
                ]);

                const usersData = usersSnap.val() || {};
                const emailToUid = emailToUidSnap.val() || {};
                const logs = loginLogsSnap.val() || {};

                // Use a map to track existing users by email (case-insensitive)
                const existingEmails = new Map();
                Object.values(usersData).forEach(u => {
                    if (u && u.email) existingEmails.set(u.email.toLowerCase(), true);
                });

                let recoveredCount = 0;
                const batchUpdates = {};

                // 1. Recover from email_to_uid mappings
                for (const [encEmail, uid] of Object.entries(emailToUid)) {
                    const email = _dec(encEmail);
                    if (email && !existingEmails.has(email.toLowerCase())) {
                        const stub = {
                            email: email,
                            name: email.split('@')[0],
                            points: 100, // Give them 100 seeds as a "welcome/found" gift if they had none
                            joined: 'Legacy / Found',
                            uid: uid,
                            status: 'Recovered'
                        };
                        batchUpdates[`users/${uid}`] = stub;
                        batchUpdates[`users/${encEmail}`] = stub;
                        existingEmails.set(email.toLowerCase(), true);
                        recoveredCount++;
                    }
                }

                // 2. Recover from login_logs (last 2000 entries)
                for (const log of Object.values(logs)) {
                    if (log && log.email && !existingEmails.has(log.email.toLowerCase())) {
                        const encEmail = _enc(log.email);
                        const stub = {
                            email: log.email,
                            name: log.name || log.email.split('@')[0],
                            points: 100,
                            joined: log.date || 'Legacy / Log',
                            status: 'Recovered from Login'
                        };
                        const uid = 'recovered_' + encEmail;
                        batchUpdates[`users/${uid}`] = stub;
                        batchUpdates[`users/${encEmail}`] = stub;
                        existingEmails.set(log.email.toLowerCase(), true);
                        recoveredCount++;
                    }
                }
                ...
                if (recoveredCount > 0) {
                    ...
                    console.log(`✅ BaqdDB: Deep Sync complete. Recovered ${recoveredCount} users!`);
                } else {
                    console.log('✅ BaqdDB: No missing accounts found.');
                }

                return { success: true, count: recoveredCount };
            } catch (e) {
                console.error('BaqdDB.deepSyncUsers error:', e.message);
        ...
        /** ✅ DEEP SYNC: Reconstruct missing user records from Login Logs */
        async deepSyncUsers() {
            if (!_db) return { success: false, error: 'Database not connected' };
            try {
                // 1. Get all existing users from Firebase
                const usersSnap = await _db.ref('users').get();
                const usersData = usersSnap.val() || {};
                const userEmails = new Set();

                Object.values(usersData).forEach(u => {
                    if (u && u.email) userEmails.add(u.email.toLowerCase());
                });

                // 2. Get all login logs
                const logsSnap = await _db.ref('login_logs').get();
                if (!logsSnap.exists()) return { success: true, count: 0 };

                const logs = Object.values(logsSnap.val() || {});
                const missingUsersMap = {};

                // 3. Find unique emails in logs that don't have a user record
                logs.forEach(log => {
                    const email = (log.email || '').trim().toLowerCase();
                    if (email && email !== 'undefined' && email !== 'null' && !userEmails.has(email)) {
                        if (!missingUsersMap[email]) {
                            missingUsersMap[email] = {
                                name: log.name || 'Recovered User',
                                email: log.email, // Preserve original casing
                                points: 0,
                                joined: log.date || new Date().toLocaleString('ar-JO'),
                                lastActive: log.timestamp || Date.now(),
                                isVerified: false,
                                isBanned: false,
                                profilePic: log.profilePic || '',
                                googleId: log.method === 'Google' ? 'recovered' : null,
                                deviceId: log.deviceFp || '',
                                authMethod: log.method || 'Unknown',
                                recovered: true
                            };
                        }
                    }
                ...
                const missingUsers = Object.values(missingUsersMap);
                if (missingUsers.length === 0) return { success: true, count: 0 };

                // 4. Batch save to Firebase
                const promises = missingUsers.map(async (u) => {
                    const uid = u.googleId === 'recovered' ? 'rec_' + _enc(u.email) : 'temp_' + _enc(u.email);
                    u.uid = uid;

                    // We save in three places to ensure visibility in all versions of the app
                    await _db.ref(`users/${uid}`).set(u);
                    await _db.ref(`email_to_uid/${_enc(u.email)}`).set(uid);
                    await _db.ref(`users/${_enc(u.email)}`).set(u);
                });
                ...
                return { success: true, count: missingUsers.length };
            } catch (e) {
